...
export default function Details(props) {
  ...
  const clickHandelerAdd = async()=>{
    await props.state.web3.eth.sendTransaction({from:props.address,to:"0x59eb1ef3cb9b4ddc7560b65debb0d9096036c9df",value:props.state.web3.utils.toWei(add,"ether")});
  ...
  const clickHandelerWid = async()=>{
    ...
    await props.state.contract.methods.Withdrawal(amount).call({ from: props.address }).then((data)=>console.log(data))
    console.log("click");
    props.state.contract.methods.players(props.address).call().then((balance)=>{props.setBalance(props.state.web3.utils.fromWei(balance,"ether"))})
  }
